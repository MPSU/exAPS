...
`ADDI` суммирует 12-разрядное непосредственное значение со знаковым расширением к регистру `rs1`. Арифметическое переполнение игнорируется, и результатом являются младшие 32 бита результата. Команда `ADDI rd, rs1, 0` используется для реализации псевдоинструкции `MV rd, rs1` на ассемблере.
