...
Опишем его на verilog в виде отдельного модуля со входами и выходами, для наглядности. Для этого нам потребуется 5 входов: 4 входа для четырехбитных мультиплексируемых сигналов и один двухбитный вход для управления, и один четырехбитный выход. Особенность языка такая, что конструкция `always` требует использования сигнала типа `reg` слева от знака `=` внутри нее. Даже, если в результате такого описания не появится ни один регистр. Так как `y` слева от знака равно внутри `always`, то он должен быть помечен как `reg`, хотя, еще раз – он не будет регистром в этом случае, потому что `always @ *` это комбинационная схема. Такие вот особенности. В итоге, код ниже превратится в картинку выше. Оператор `case` "выберет" ветку в зависимости от значения в скобках. Для данного примера, если `s == 2'b10`, то к выходу `y` подключается значение входа `d2`.
...
Рассмотрим еще один пример, в котором нет желания использовать все входы мультиплексора, которых должно быть 2 в степени "количества бит сигнала управления". В этом случае **ОБЯЗАТЕЛЬНО** использование конструкции `dеfаult:`. Если ее не добавить, то на выходе мультиплексора появится [защелка](https://en.wikipedia.org/wiki/Flip-flop_(electronics)#SR_NOR_latch) (`latch`). Защелка будет рассмотрена на [`лекции 5`](../../Lectures/05.%20Sequencial%20logic.md), в рамках лабораторных работ она ни разу не применяется.
...
С `dеfinе` гораздо взрослее, серьезнее и понятнее смотрится. Кстати, сразу на заметку директива `ˋinclude`. С помощью нее можно вставлять код из одного файла, в код другого подобно `#include` на языке `C`.
...
Вторая пара инструкций вычисляет "ветвительный" результат. Результат операции будет подан на выходной сигнал `Flag` и будет использован при ветвлении.  
...
Чтобы подгрузить `define`, скачайте файл `def𝚒nes_r𝚒sсv.v` из папки с лабой и добавьте в проект как `source`, либо создайте новый .v файл с соответсвующим названием и скопируйте в него все содержимое файла. Чтобы он правильно работал, выберите его в окне проекта `Sources`, скорее всего, он будет лежать в папке `Non-module Files`, и в окне `Source File Properties` измените `Type` с `Verilog` на `Verilog Header`. В результате строка `include "defines_riscv.v"` не должна выдавать ошибку.
...
3. После реализации модуля АЛУ его нужно будет проверить с помощью [`тестового окружения`](../../Other/Testbench.md).
