...
Разрабатываемый АЛУ будет выполнять 16 различных операций. Для кодирования 16 операций было бы достаточно 4 бит, но в лабораторной работе предлагается использовать 5-битный код, что связано с особенностями кодирования инструкций RISC-V (будет рассмотрено в лекциях 5 и 6). Видно, что старший бит кода операции указывает на то является ли операция вычислительной, или это операция сравнения.

|ALUOp|={flag, add/sub, aluop}|Result                           |Flag                            |Операция                          |
|-----|-----------------------|---------------------------------|--------------------------------|----------------------------------|
|`ADD |         0 0 000       |Result = A + B                   | Flag = 0                       | Сложение                         |
|`SUB |         0 1 000       |Result = A - B                   | Flag = 0                       | Вычитание                        |
|`SLL |         0 0 001       |Result = A << B                  | Flag = 0                       | Сдвиг влево                      |
|`SLT |         0 0 010       |Result = $signed(A) < $signed(B) | Flag = 0                       | Знаковое сравнение               |
|`SLTU|         0 0 011       |Result = A < B                   | Flag = 0                       | Беззнаковое сравнение            |
|`XOR |         0 0 100       |Result = A ^ B                   | Flag = 0                       | Побитовое исключающее **ИЛИ**    |
|`SRL |         0 0 101       |Result = A >> B                  | Flag = 0                       | Сдвиг вправо                     |
|`SRA |         0 1 101       |Result = $signed(A) >>> B        | Flag = 0                       | Арифметический сдвиг вправо      |
|`OR  |         0 0 110       |Result = A \| B                  | Flag = 0                       | Побитовое логическое **ИЛИ**     |
|`AND |         0 0 111       |Result = A & B                   | Flag = 0                       | Побитовое логическое **И**       |
|`BEQ |         1 1 000       |Result = 0                       | Flag = (A == B)                | Выставить флаг, если **равны**   |
|`BNE |         1 1 001       |Result = 0                       | Flag = (A != B)                | Выставить флаг, если **не равны**|
|`BLT |         1 1 100       |Result = 0                       | Flag = $signed(A) < $signed(B) | Знаковое сравнение **<**         |
|`BGE |         1 1 101       |Result = 0                       | Flag = $signed(A) ≥ $signed(B) | Знаковое сравнение **≥**         |
|`BLTU|         1 1 110       |Result = 0                       | Flag = A < B                   | Беззнаковое сравнение **<**      |
|`BGEU|         1 1 111       |Result = 0                       | Flag = A ≥ B                   | Беззнаковое сравнение **≥**      |
