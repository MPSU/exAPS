...
С `dеfinе` гораздо взрослее, серьезнее и понятнее смотрится. Кстати, сразу на заметку директива `ˋinclude`. С помощью нее можно вставлять код из одного файла, в код другого, подобно `#include` на языке `C`. [`Инстуркция по подключению Verilog Header`](../../Other/Verilog%20Header.md).
...
Чтобы подгрузить `define`, воспользуйтесь [`инстуркцией по подключению Verilog Header`](../../Other/Verilog%20Header.md).
...
3. После реализации модуля АЛУ его нужно будет проверить с помощью [`тестового окружения`](../../Other/Testbench.md), которое находится [`здесь`](tb_miriscv_alu.v).
