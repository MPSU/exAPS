...
3. В соответствии с [`инструкцией по подключению Verilog Header`](../../Other/Vivado%20Basics/Verilog%20Header.md) подключить в проект заголовочный файл [defines_riscv.v](defines_riscv.v).
4. Внимательно ознакомиться с заданием (раздел [#задание](#задание))
5. В уже созданном проекте создать новый Verilog-файл, и описать в нем модуль АЛУ с таким же именем и входами, как указано в конце задания.
6. Проверить описанный модуль с помощью тестового окружения ([tb_miriscv_alu.v](tb_miriscv_alu.v)).
7. Добавить в проект модуль верхнего уровня ([nexys_alu.v](board%20files/nexys_alu.v)), соединяющий АЛУ с периферией в ПЛИС.
8. Подключить к проекту файл ограничений ([nexys_a7_100t.xdc](board%20files/nexys_a7_100t.xdc)
9. Проверить работу АЛУ в ПЛИС.

Обратите внимание, что схематик в конце задания приведен в качестве референса, при выполнении задания вы должны в первую очередь ориентироваться на таблицу, в которой описаны поддерживаемые АЛУ операции, а не на эту схему.
