...
Особенность языка такая, что **конструкция `always` требует использования `reg`** слева от знака равно внутри нее. **Даже если в результате такого описания не появится ни один регистр. Так как `y` слева от знака равно внутри `always`, то он должен быть помечен как `reg`, хотя, еще раз – он не будет регистром в этом случае, потому что `always @ *` это комбинационная схема.**
...
**Поскольку мы будем реализовывать АЛУ для процессора архитектуры rv32i (который работает с 32-битными целочисленными значениями), для ВСЕХ операций сдвига мы должны брать только 5 младших бит операнда B.** 
...
Чтобы подгрузить `define`, воспользуйтесь [`инструкцией по подключению Verilog Header`](../../Other/Vivado%20Basics/Verilog%20Header.md).

### Порядок выполнения задания

1. В `Design Sources` проекта создайте `Verilog`-файл `ALU.v`.
2. Опишите в файле модуль `аlu_r𝚒sсv`, схема которого представлена на рисунке выше.Поскольку у вас два выходных сигнала, зависящих от сигнала ALUop, вам потребуется описать два разных мультиплексора. Будьте внимательны с назначением выходных значений мультиплексоров при использовании адресов, не использующиеся в их логике. Обратите внимание на размерности входных, выходного и управляющего сигналов.
