...
Используя навыки по описанию мультиплексоров, писать блок арифметико-логического устройства (АЛУ) на языке Verilog.

## Допуск к лабораторной работе

Освоить [описание мультиплексора на языке Verilog](../../Other/Basic%20Verilog%20structures/Multiplexors.md).
...
В случае этого АЛУ будет удобно использовать `dеf𝚒nе` для обозначения кодов команд. Во-первых, для того чтобы в `case` не допустить ошибок, а во-вторых – чтобы можно было легко менять управляющие коды для повторного использования АЛУ в других проектах.
...
Кроме озвученного, на языке Verilog также потребуется использовать [операции сдвига](https://ru.wikipedia.org/wiki/Битовый_сдвиг), к которым относятся:
...
Необходимо на языке Verilog реализовать АЛУ в соответствии со следующим прототипом:
...
module аlu_r𝚒sсv (
  ...
  𝚒nput       [31:0]  b_i,
  𝚒nput       [4:0]   alu_op_i,
  оutput  rеg         flag_o,   // reg, потому что основой АЛУ будет два
  оutput  rеg [31:0]  result_o  // мультиплексора, описанных в case внутри always,
);                              // а в always, слева от "равно", всегда стоит
                                // сигнал, объявленный как reg
...
4. После реализации модуля АЛУ его нужно будет проверить с помощью тестового окружения.
   1. Добавьте файл [`tb_miriscv_alu.v`](tb_miriscv_alu.v) в `Simulation sources`.
   ...
   4. Убедитесь, что симуляция завершена (об этом будет соответствующее сообщение в консоли). По завершению симуляции, в случае отсутствия ошибок, будет выведено сообщение "SUCCESS", в противном случае будут выведены сообщениия об этих ошибках.
   5. В случае, если были найдены ошибки, вы должны найти и исправить их. Для этого руководствуйтесь [документом](../../Other/Vivado%20Basics/Debug_manual.md).
