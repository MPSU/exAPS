...
Сравнивая блочную память с распределенной/регистровой: поскольку большой объем памяти съест много логических блоков при реализации распределенной/регистровой памяти, такую память лучше делать в виде блочной.
...
1. На языке Verilog необходимо реализовать модуль памяти команд (`instr_mem1024_8`). Память представляет собой однопортовую ПЗУ с 1024-ю 8-битными ячейками. Созданную память необходимо инициализировать данными, воспользовавшись [`Instruction_Memory.txt`](Instruction_Memory.txt), после чего проверить в [`testbench`](tb_instr_mem.v).

![Память инструкций](../../../technical/Labs/Pic/im.png)

2. На языке Verilog необходимо реализовать модуль памяти данных (`data_mem1024_8`) для процессора с архитектурой RISC-V. Память представляет собой однопортовую ОЗУ с 1024-ю 8-битными ячейками, но имеющая 32-битную шину адреса. То есть реально будет использоваться только ceil(log2(1024)) = 10 из 32 битов входного адреса `RAM[A[9:0]]`, вместо `RAM[A]`. Созданную память необходимо проверить в [`testbench`](tb_data_mem.v).

![Память данных](../../../technical/Labs/Pic/dm.png)

3. На языке Verilog необходимо реализовать модуль регистрового файла (`rf_r𝚒sсv`) для процессора с архитектурой RISC-V, представляющего собой трехпортовую ОЗУ с двумя портами на чтение и одним портом на запись и состоящей из 32-х 32-битных регистров, при этом по адресу `0` должно всегда считываться значение `0` вне зависимости от того, какое значение в этой ячейке памяти, и есть ли она вообще. Такая особенность обусловлена тем, что при выполнении операций ноль очень часто используется (сравнение с нулем, инициализация переменных нулевым значением, копирование значения одного регистра в другой посредством сложения с нулем и записи результата и т.п.). Для реализации такого функционала можно использовать мультиплексоры, стоящие между регистровым файлом и выходными портами на чтение, и определяющие по адресам чтения, какие данные пойдут на выход: 0 или данные из регистрового файла. Либо же можно проинициализировать нулевую ячейку памяти нулем с запретом записи в неё каких-либо значений. В этом случае в ячейке всегда будет ноль, а значит и считываться с нулевого адреса будет только он. Созданный регистровый файл необходимо проверить в [`testbench`](tb_reg_file.v).
