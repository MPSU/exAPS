...
С учетом названия исполняемого названия скачанного вами компилятора (при условии, что вы установили его по предложенному установщиком пути), командой для компиляции файла `startup.S` может быть:
...
В процессе отладки лабораторной работы потребуется много раз смотреть на программный счетчик и текущую инструкцию. Декодировать инструкцию самостоятельно, чтобы понять что сейчас выполняется довольно тяжело. Для облегчения задачи можно дизасемблировать скомпилированный файл. Полученный файл на языке ассемблера будет хранить адреса инструкций, а так же их двоичное и ассемблерное представление.
...
```bash
/c/sysgcc/risc-v/bin/riscv64-unknown-elf-objdump -D result.elf disasmed_result.S
```
