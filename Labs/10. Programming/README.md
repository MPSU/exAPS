...
<details>
  ...
  objcopy должен понять по ключу --verilog-data-width=4, что адреса ячеек в файле должны быть разделены на 4. Ход работ над данным багом можно увидеть [здесь]    (https://sourceware.org/bugzilla/show_bug.cgi?id=27214). Не смотря на то, что за последние два года баг так и не перевели в состояние accepted, сам он исправлен в ветке master и когда-нибудь это исправление попадет в кросс-компилятор. Когда это произойдет, данный спойлер можно будет убрать.
