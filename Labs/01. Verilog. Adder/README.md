...
Реализуем схему (рис.3) в виде модуля, описанного на языке Verilog:

<br>

![](../../../technical/Labs/Pic/example.drawio.png)

*Рисунок 3. Схема модуля example*

Модуль example имеет три входных сигнала и один выходной. Для удобства понимания в листинге кода присутствуют сигналы n1—n4. Их можно опустить, тогда s4 будет задаваться так: `assign s4 = (~(s2 & s3) & s1) ^ ((s1 ^ s2) | s1)`.

```systemverilog
module example(
  input      s1,   // Входные сигналы
  input      s2,
  input      s3,
  output     s4    // Выходной сигнал
...
```
*Листинг 1. Verilog-код модуля example*

По данному коду, САПР сможет реализовать следующую схему:

![](../../../technical/Labs/Pic/example_vivado.png)

*Рисунок 4. Цифровая схема модуля example, сгенерированная САПР Vivado*
...
module testbench();             // <- Не имеет ни входов, ни выходов!
    ...
    initial begin
        s1 = 'd0; s2 = 'd0; s3 = 'd0;  // <- Подаём на входы модуля тестовые
        #10;                           //    воздействия
        s1 = 'd0; s2 = 'd0; s3 = 'd1;
        #10;                           // <- Делаем паузу в десять отсчётов
        s1 = 'd0; s2 = 'd1; s3 = 'd0;  // времени симуляции перед очередным
...
*Рисунок 5. Временная диаграмма, моделирующая работу схемы с рис.3*

В данной лаборатной работе вам предстоит реализовать схему полного однобитного сумматора (рис.2).
...
*Рисунок 6. Схема четырехбитного сумматора*
...
4. Опишите модуль `fulladder32` таким образом, чтобы в нем выполнялось поразрядное сложение двух 32-битных чисел с помощью многократного подключения модуля, описанного в файле `fulladder.v`.
