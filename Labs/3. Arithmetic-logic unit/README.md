...
На языке Verilog описать блок арифметико-логического устройство (АЛУ) и проверить работу на отладочном стенде.
...
4. Создать новый проект для всего цикла лабораторных работ (пример в [`Создание базового проекта с прошивкой ПЛИС в Vivado`](../../Other/Vivado-trainer.md))
5. Описать АЛУ на языке verilog (практическая работа)
6. Проверить работу АЛУ на ПЛИС (подключить переключатели и светодиоды ко входам и выходам)
...
После реализации модуля АЛУ его нужно будет проверить на отладочном стенде используя 5 переключателей для обозначения операции, а остальные 11 разделить между младшими битами входов `A` и `B`. Младшие биты выхода результата и флага подключить к светодиодам.
