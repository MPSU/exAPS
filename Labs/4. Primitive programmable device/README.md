...
2. Найти количество единиц в двоичном представлении числа `IN` (обрати внимание, `IN` – знаковое число). Вывести результат в `OUT`.
Пример: `IN = 0101_0110`. Результат вычислений: `OUT = 100`.    

3. Вычислить [циклический сдвиг](https://ru.wikipedia.org/wiki/Битовый_сдвиг#Циклический_сдвиг) влево `a` << `IN`. Вывести результат в `OUT`.
Пример: `a` = `01011`, `IN` = `10`. Результат вычислений: `OUT = 01101`.    

4. Если `IN[1:0] == 0`, то в `OUT` выводится `a`,  если `IN[1:0] == 1`, то `b`,  если `IN[1:0] == 2`, то `c`,  если `IN[1:0] == 3`, то `d`.
Пример: `a = 00`, `b = 10`, `c = 11`, `d = 01`, `IN = 01`. Результат вычислений: `OUT = 10`.  

5. Если `IN` является степенью двойки, то вывести `OUT =  1`, в противном случае, `OUT = 0`.
Пример 1: `IN = 100`. Результат вычислений: `OUT = 1`.   
Пример 2: `IN = 110`. Результат вычислений: `OUT = 0`.   

6. Вычислить `a * IN` посредством суммы `IN` значений `a`. Вывести результат в `OUT`.
Пример: `a = 5`, `IN = 4`. `5 * 4 == 5 + 5 + 5 + 5 = 20`.    

7. Найти количество нулей в двоичном представлении числа `IN`. Вывести результат в `OUT`.
...
9. Вычислить `a - IN` без использования операции вычитания. Вывести результат в `OUT`.
Пример: `IN = 011`, `a = 100`. Результат вычислений: `001`.   

10. Найти наибольший двоичный разряд числа `IN`, значение которого равно `1`. Если такого нет, вывести `32`. Вывести результат в `OUT`.
Пример: `IN = 0110`. Результат вычислений: `OUT = 10`.    
   
11. Найти количество двоичных разрядов, в которых различаются числа `IN` и `a`. Вывести результат в `OUT`.
Пример: `IN = 0110`, `a = 1110`. Результат вычислений: `OUT = 01`.   

12. Вывести в `OUT` подряд все единицы входного числа `IN`. 
Пример: `IN = 1011011011`. Результат вычислений:`OUT` = `1111111`   
---

13. Найти и вывести в `OUT` количество вхождений `a[2:0]` в `IN` без пересечений.
Пример: `a[2:0] = 010`, `IN = 1101_0101`. Результат вычислений: `OUT = 01`.   

14. Определить, сколько раз встречается `11` в двоичном представлении `IN` без пересечений. Вывести результат в `OUT`.
Пример: `IN = 01110`. Результат вычислений: `OUT = 01`.   

15. Найти остаток от деления `IN` на `a`. Вывести результат в `OUT`.
Пример: `IN = 101`, `a = 10`. Результат вычислений: `OUT = 01`.   

16. Вывести в `OUT` сумму `IN[3:0]` + `IN[7:4]` + `IN[11:8]` + `IN[15:12]`.
Пример: `IN = 0001_0010_00011_0000`. Результат вычислений: `OUT = 110`.   

17. Вывести в `OUT` результат целочисленного деления `a`/`IN`.
Пример: `IN = 10`, `a = 111`. Результат вычислений: `OUT = 11`.   

18. В числе `IN` заменить справа-налево каждое `00` на `11`. Вывести результат в `OUT`.
Пример: `IN = 101000`. Результат вычислений: `OUT = 101011`.   
---

19. Поменять местами чётные биты числа `IN` с нечётными битами этого числа (то есть соседние биты поменять местами). Вывести результат в `OUT`.
Пример: `IN = 1010_0111`. Результат вычислений: `OUT = 0101_1011`.   

20. Инвертировать первые `IN` бит числа `a`. Вывести результат в `OUT`.
Пример: `IN = 11`, `a = 1010_0011`. Результат вычислений: `OUT = 1010_0100`.   

21. Вывести n-ый член [последовательности Фибоначчи](https://ru.wikipedia.org/wiki/Числа_Фибоначчи) Fn. n = `IN`. Вывести результат в `OUT`.
Пример: `IN = 100`. Результат вычислений: `OUT = 10`.   

22. Поменять в числе `a` разряды `i = IN[4:0]` и `j = IN[9:5]`. Вывести результат в `OUT`.
Пример: `a = 1001`, `IN[9:0] = 00000_00001`. Значит, в числе `а` необходимо поменять местами `а[0]` и `a[1]`. Результат вычислений: `OUT = 1010`.
---

23. Вычислить `a * IN` с использованием операций сложений и сдвига ("в столбик"). Вывести результат в `OUT`. [Пример](https://studfile.net/preview/8921792/page:5/)   

24. Вывести в `OUT` n-ый член [арифметической прогрессии](https://ru.wikipedia.org/wiki/Арифметическая_прогрессия) aN, где a1 = `a`, d = `IN[15:8]`, n = `IN[7:0]` (d и n неотрицательные).
