...
![Схема процессорной системы](../../../technical/Labs/Pic/riscv_periph_uarch.drawio.png)
...
На приведенной выше схеме используется второй вариант — устройство понимает, что к нему обратились по специальному сигналу `req_i`. Данный сигнал формируется из двух частей: сигнала `req` исходящего из процессорного ядра (сигнал о том, обращение в память вообще происходит) и специального сигнала-селектора исходящего из 256-разрядной шины. Формирование значения на этой шине происходит с помощью onehot-кодирования. Процесс кодирования достаточно прост. В любой момент времени на выходной шине должен быть **ровно один** бит, равный единицы. Индекс этого бита совпадает со значением старших восьми бит адреса. Поскольку для восьмибитного значения существует 256 комбинаций значений, именно такая разрядность будет на выходе кодировщика. Это означает, что в данной системе можно связать процессор с 256 устройствами (одним из которых будет память данных).

Реализация такого кодирования предельно проста:

Нулевой сигнал этой шины будет равен единице только если старшие восемь бит адреса равны нулю.  
Первый бит этой шины будет равен единицы только если старшие  восемь бит адреса равны единицы.  
...  
Двести пятьдесят пятый бит шины будет равен единице только если старшие восемь бит адреса равны двумстам пятидесяти пяти.

Для реализации такого кодирования необходимо сделать 256 однотипных непрерывных присваиваний, реализацию которых можно автоматизировать через конструкцию `generate for`, пример использования которой приводился в первой лабораторной работе.

Однако в данной лабораторной работе вам будет необходимо подключить к процессору всего два периферийных устройства (помимо памяти данных), поэтому для работы системы достаточно реализовать три первых бита из шины-селектора. Это можно сделать и с помощью трех непрерывных присваиваний.
...
3. Те кому достались варианты 16-29 должны реализовать контроллеры приемника и передатчика UART.
...
1. Внимательно ознакомьтесь описанием контроллеров периферии своего варианта. В случае возникновения вопросов, проконсультируйтесь с преподавателем.
2. Реализуйте модули контроллеров периферии. Имена модулей и их порты будут указаны в [описании контроллеров](#описание-контроллеров-периферийных-устройств).
3. Интегрируйте модули контроллеров периферии в процессорную систему по приведенной схеме руководствуясь старшими адресами контроллеров, представленными на карте памяти. Это означает, что если вы реализуете контроллер светодиодов, на его входов req_i должна подаваться единица в случае если `mem_req_o == 1` и старшие 8 бит адреса равны `0x02`.
4. Проинициализируйте память инструкций с помощью предоставленной для каждой пары контроллеров программы.
...
6. Проверьте работу вашей процессорной системы с помощью отладочного стенда с ПЛИС и (при соответствующем варианте) клавиатуры/рабочего компьютера.
...
1. Под "запросом на запись по адресу `0xАДРЕС`" будет пониматься совокупность следующих условий:
   ...
   3. На входе `WE_i` выставлено значение `1`.
...
2. Под "запросом на чтение по адресу `0xАДРЕС`" будет пониматься совокупность следующих условий:
   ...
   2. На входе `WE_i` выставлено значение `0`.
   3. На входе `addr_i` выставлено значение `0xАДРЕС`

Обратите внимание на то, что запрос на чтение работает вне зависимости от тактового синхроимпульса (реализуется асинхронное чтение). Это необходимо, чтобы наша реализация однотактного процессора работала корректно.
...
В случае отсутствия запроса на чтения, на выход `RD_o` должно подаваться значение `32'hZZZZZZZZ`. Это никак не повлияет на работу процессора, но будет удобно в процессе отладки на временной диаграмме.

Если пришел запрос на запись или чтение, это еще не значит, что контроллер должен его выполнить. В случае, если запрос происходит по адресу, не поддерживающему этот запрос (например запрос записи по адресу поддерживающему только чтение или наоборот), данный запрос должен игнорироваться, а на выходе `RD_o` должно остаться значение `32'hZZZZZZZZ`.

В случае осуществления записи по принятому запросу, необходимо записать данные с сигнала `WD_i` в регистр, ассоциированный с адресом `addr_i` (если разрядность регистра меньше разрядности сигнала `WD_i`, старшие биты записываемых данных отбрасываются).

В случае осуществления чтения по принятому запросу, необходимо выставить данные с сигнала, ассоциированного с адресом `addr_i` на выходной сигнал `RD_o` (если разрядность сигнала меньше разрядности выходного сигнала `RD_o`, возвращаемые данные должны дополниться нулями в старших битах).
...
По сути, логика работы контроллера сводится к тому, выдавать на шину `RD_o` данные со входа `sw_i` каждый раз, когда приходит запрос на чтение по нулевому адресу. Поскольку разрядность `sw_i` в два раза меньше разрядности выхода `RD_o` его старшие биты необходимо дополнить нулями.
...
module PS2Receiver(
    ...
    output [7:0]  keycodeout,   // Сигнал полученного с клавиатуры скан-кода клавиши
...
По каждому восходящему фронту сигнала `clk_i` вы должны проверять выход `keycode_valid` и если тот равен единице, записать значение с выхода `keycodeout` в регистр `scan_code`. При этом значение регистра `scan_code_is_unread` необходимо выставить в единицу.

В случае, если произошел запрос на чтение по адресу `0x00`, необходимо выставить на выход `RD_o` значение регистра `scan_code` (дополнив старшие биты нулями), при этом значение регистра `scan_code_is_unread` необходимо обнулить.

В случае запроса на чтение по адресу `0x04` необходимо вернуть значение регистра `scan_code_is_unread` изменения состояний регистров.
...
Обратите внимание на то, что разрядность входов hex0-hex7 составляет 5 бит — это на 1 бит больше, чем нужно для указания одной шестнадцатеричной цифры. Старший бит входа отвечает за "включение"/"выключение" индикатора. Когда старший бит равен единице, на индикатор будет выведена цифра, указанная в младших четырех битах, когда он равен нулю — семисегментный индикатор погаснет. Т.е. если на вход `hex0` подать число `5'b10011`, на самом правом индикаторе загорится цифра `3`. Если же подать число `5'b00011` (или любое другое число в диапазоне [`0:15]`) — этот индикатор погаснет.
...
Регистры `hex0-hex7` отвечают за вывод цифры на соответствующий семисегментный индикатор. Регистр `bitmask_on_off` отвечает за включение/отключение семисегментных индикаторов. Когда в регистре `bitmask_on_off` бит, индекс которого совпадает с номером индикатора равен единице — тот включен и выводит число, совпадающее со значением в соответствующем регистре `hex0-hex7`. Когда бит равен нулю — этот индикатор гаснет.

Доступ на чтение/запись регистров `hex0-hex7` осуществляется по адресам `0x00-0x1c` (см. таблицу адресного пространства).

Доступ на чтение/запись регистра `bitmask_on_off` осуществляется по адресу `0x20`.

При запросе на запись единицы по адресу `0x24` необходимо выполнить сброс всех регистров. При этом регистр `bitmask_on_off` должен сброситься в значение `0xFF`.

Адресное пространство контроллера:

|Адрес|Режим доступа|Допустимые значения|                 Функциональное назначение               |
|-----|-------------|-------------------|---------------------------------------------------------|
|0x00 | RW          | [0:32]            | Регистр, хранящий значение, выводимое на hex0           |
|0x04 | RW          | [0:32]            | Регистр, хранящий значение, выводимое на hex1           |
|0x08 | RW          | [0:32]            | Регистр, хранящий значение, выводимое на hex2           |
|0x0C | RW          | [0:32]            | Регистр, хранящий значение, выводимое на hex3           |
|0x10 | RW          | [0:32]            | Регистр, хранящий значение, выводимое на hex4           |
|0x14 | RW          | [0:32]            | Регистр, хранящий значение, выводимое на hex5           |
|0x18 | RW          | [0:32]            | Регистр, хранящий значение, выводимое на hex6           |
|0x1C | RW          | [0:32]            | Регистр, хранящий значение, выводимое на hex7           |
...
Сигнал `rst_i` должен быть равен единице при запросе на запись единицы по адресу `0x24` (в остальных случаях он равен нулю).
...
|0x10 | RW          | [0:32]            | Чтение/запись регистра `parity`, отвечающего за включение отключение проверки данных через бит четности |
|0x14 | RW          | [0:32]            | Чтение/запись регистра `stopbit`, отвечающего за длину стопового бита                                   |
|0x24 | W           |  1                | Запись сигнала сброса                                                                                   |
