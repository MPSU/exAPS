...
Обратите внимание на то, что изменился сигнал сброса (`resetn`). Буква `n` на конце означает, что сброс работает по уровню `0` (когда сигнал равен нулю — это сброс, когда единице — не сброс). Чтобы не менялась логика работы сброса ваших предыдущих модулей, необходимо создать сигнал `rst_i`, который является инверсией входа `resetn`.
...
1. Подключить файл `clk_div.v` в ваш проект.
...
3. Обновите модуль `riscv_unit` в соответствии с разделом ["Дополнительные правки модуля riscv_unit"](#дополнительные-правки-модуля-riscv-unit).
   1. Подключите в проект файл `clk_div.v`.
